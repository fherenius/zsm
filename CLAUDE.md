...
2. **`update(event)`** — handles `Key`, `SessionUpdate`, `PaneUpdate`, `TabUpdate`, `ModeUpdate`, `RunCommandResult`, `PermissionRequestResult`, `Visible`, and `Timer`. Returns `bool` = "should re-render". Fetches wait for `PermissionStatus::Granted`. Pane/tab reports detect restored suppressed panes and refocusing: Zellij can omit `Visible(true)` on these paths. Closing the picker clears search immediately; reopening/refocusing re-queries zoxide, sessions, and shared visit history. A single timer refreshes sessions every second while visible, with at most one CLI listing in flight. `zellij list-sessions --no-formatting` provides socket-confirmed live membership and resurrectable sessions; `get_session_list` provides details but can omit sessions with missing/unreadable metadata. Passive `SessionUpdate` events update only current-session details: their peer cache can be stale, and fetching directly in response would create an event loop in Zellij 0.45.1. **Naming and the merge re-run every time the plugin is shown** — keep both cheap.
